...
module VConsole::Commands # .class_eval <<END
  ...
  vars['/mdk/merchant_detail/'] = {:timeout=>15, :conn=>"get", :url=>"http://localhost:3000"}
  ...
  #env_start
  @@history = ["rm /d", "rm /stripe/token/a/", "help", "c", "create /a/b/c", "rm /a", "create a/b/c", "rm a/b/c", "rm a", "cd tw", "headers wow=kereen", "cp /c .", "cp /c/d .", "cp /c/d c", "cp /c/d c/d", "rm /twitter/c", "rm /twitter/d", "cd", "create mdk", "cd mdk", "create merchant/new", "set url=http//10.2.250.128080/mdk/rest/merchant/new", "set conn=post", "set url=http://10.2.250.12:8080/mdk/rest/merchant/new", "headers", "send /twitter/", "send /twitter/b", "l", "rm merchant/new", "create merchant_new", "clear", "h", "send", "cd _new", "reset", "cd ..", "create merchant_detail", "rm merchant/", "cd new", "set url = http://10.2.250.12:8080/mdk/rest/merchant/new", "set body={\"id\":\"merchant id\",\"ccid\":\"merchant ccid\",\"hash\":\"merchant hash\"}", "set body={}", "set body={\"id\":\"wow kereen\"}", "set", "ls", "set body = {\"id\":\"merchant id\",\"ccid\":\"merchant ccid\",\"hash\":\"merchant hash\"}", "info", "save"]
